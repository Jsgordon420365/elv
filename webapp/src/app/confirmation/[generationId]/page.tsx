        ...
        <main className="min-h-screen bg-[#0a0a0c] text-slate-100">
            ...
            <div className="mx-auto max-w-5xl space-y-8 px-6 py-10">
                {!assurance ? <p className="text-slate-400">Unlocking assurance record…</p> : <>
                    ...
                    <section className="flex flex-wrap gap-3"><button onClick={downloadStoredDocument} className="flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-3 text-sm font-bold hover:bg-slate-800"><Download className="h-4 w-4" />Download this DOCX</button><button onClick={regenerate} disabled={loading} className="flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold hover:bg-indigo-500 disabled:opacity-50">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}Regenerate with current vault facts</button><Link href={`/recourse/${assurance.generationId}`} className="flex items-center gap-2 rounded-xl bg-red-600 px-4 py-3 text-sm font-bold hover:bg-red-500"><Siren className="h-4 w-4" />This document failed, was rejected, or did not perform as represented.</Link></section>
                </>}
            </div>
            <UnlockOverlay />
        </main>
